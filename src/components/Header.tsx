...
export const Header: React.FC = () => {
  ...
  return (
    <>
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-8 text-slate-900 transition-colors duration-200 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-100">
        
        {/* Breadcrumbs */}
        <div className="flex items-center space-x-2 overflow-hidden text-xs">
          {getBreadcrumbs().map((crumb, idx) => (
            <React.Fragment key={`${crumb.label}-${idx}`}>
              {idx > 0 && <span className="text-slate-350 dark:text-neutral-700 mx-1">/</span>}
              <button
                ...
                className={`text-xs tracking-tight transition-colors duration-150 shrink-0 font-medium ${
                  idx === getBreadcrumbs().length - 1
                    ...
                    : 'text-slate-400 hover:text-slate-900 dark:text-neutral-500 dark:hover:text-neutral-300'
                }`}
              >
                {crumb.label}
              </button>
        ...
        <div className="flex items-center space-x-6">
          ...
          <button
            ...
            className="flex items-center space-x-2 rounded bg-slate-100 px-3 py-1.5 text-left text-[11px] text-slate-400 hover:bg-slate-200/60 outline-hidden w-64 transition-colors duration-150 dark:bg-neutral-800 dark:text-neutral-400 dark:hover:bg-neutral-700/60"
            title="Search (Cmd + K)"
          >
            <Search className="h-3.5 w-3.5 text-slate-400 shrink-0" />
            <span className="truncate">Search curriculum, resources, notes...</span>
            <kbd className="hidden rounded bg-slate-200 px-1 font-mono text-[9px] text-slate-500 sm:inline dark:bg-neutral-700 dark:text-neutral-400">
              ⌘K
            </kbd>
          </button>
          ...
          <div className="relative">
            ...
            >
              ...
              {notifications.some(n => !n.read) && (
                <span className="absolute top-1 right-1 h-2 w-2 rounded-full bg-slate-900 dark:bg-slate-200 border-2 border-white dark:border-neutral-900" />
              )}
            </button>

            {showNotifications && (
              <div className="absolute right-0 mt-2 z-40 w-80 rounded border border-slate-200 bg-white p-2 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
                <div className="flex items-center justify-between border-b border-slate-100 px-3 py-2 dark:border-neutral-800">
                  <span className="text-xs font-bold text-slate-900 dark:text-neutral-200">System Notifications</span>
                ...
                <div className="max-h-64 overflow-y-auto mt-1 space-y-1">
                  {notifications.map((item) => (
                    <div 
                      key={item.id} 
                      className={`p-2.5 rounded hover:bg-slate-50 transition-colors duration-150 dark:hover:bg-neutral-800/40 ${
                        !item.read ? 'bg-slate-50/50 dark:bg-neutral-800/20' : ''
                      }`}
                    >
                      <div className="flex items-start justify-between">
                        <span className="text-xs font-semibold text-slate-900 dark:text-neutral-200">{item.title}</span>
                        <span className="text-[9px] text-slate-400">{item.time}</span>
                      </div>
                      <p className="text-[11px] text-slate-500 mt-0.5 leading-normal dark:text-neutral-400">{item.body}</p>
                    </div>
                  ))}
                </div>
                <div className="mt-2 border-t border-slate-100 pt-2 text-center dark:border-neutral-800">
                  <button 
          ...
          >
            <div className="h-7 w-7 rounded-full bg-slate-200 border border-slate-300 flex items-center justify-center text-xs font-semibold text-slate-600 dark:bg-neutral-800 dark:text-neutral-300">
